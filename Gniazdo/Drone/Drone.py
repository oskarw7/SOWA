...
class Drone:
    ...
    def get_current_nema_gps_message(self) -> String:
        return self.GpsHandler.generate_nema_gga(*self._position)

    def follow_positions(self):
        self.position = [self.check_points[0][0], self.check_points[0][1], self.check_points[0][2]]
        dt = 1 / 24 
        while True:
            completed_counter = 0
            ...
            v = [
                ...
                ((( next_checkpoint[0] - last_checkpoint[0] + self.sim.Scene.image_width/2) % self.sim.Scene.image_width) - self.sim.Scene.image_width/2 ) / next_checkpoint[3],
